...
cstruct.sizeOf = function(name) {
    // get schema
    return schemeSize(cstruct.retrieve(name));
};
        ...
        for (var el in scheme) {
            ...
            if (typeof s == 'string') {
                ...
                // number
                    ...
                    if (s.indexOf('-') > -1) {
                        var xnum = s.split('-');
                        res[el] = unpacker[xnum[0] + bord](pointer, buf);
                    } else {
                        res[el] = unpacker[s + bord](pointer, buf);
                    }
                }
            } else if(s.hasOwnProperty('type') && s.type == 'dynamic'){
                for(var i = 1; i <= res[s.count]; i++){
                    res[el+i] = next(pointer, buf, cstruct.retrieve(el));
                }
        ...
        for (var el in scheme) {
            ...
            if (typeof s === 'string') {
                ...
                // number
                    ...
                    if (s.indexOf('-') > -1) {
                        var xnum = s.split('-');
                        binary.push(packer[xnum[0] + bord](obj[el] || xnum[1]));
                    } else {
                        binary.push(packer[s + bord](obj[el]));
                    }
                }
            } else if (Array.isArray(s)) {
...
unpacker.uint48b = function(pointer, buffer) {
    ...
    binary += buffer[pointer.offset++] * 0x100000000;
    binary += buffer[pointer.offset++] * 0x1000000;
    binary += buffer[pointer.offset++] << 16;
    binary += buffer[pointer.offset++] << 8;
    binary += buffer[pointer.offset++];
    return binary;
